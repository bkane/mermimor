<template>
    <div class="row bg-dark text-light">
        ...
        <div class="col h4 align-self-center pt-1 pb-0">
            <div class="vstack p-0">
                {{ name }}
                <p class="text-muted m-0 fs-6">{{ owner }}</p>
            </div>
        </div>
        <div class="col-2 h4 ps-0 text-start align-self-center pt-1">{{ score }} pts</div>
    </div>

    <div class="picks-table">
        <div class="row striped-row" v-for="pick in picks" :key="pick.id">
            <div class="col-1"></div>
            <div class="col-8 col-md py-0 ps-4" :class="{ 'text-decoration-line-through text-muted': pick.eliminated }">
                {{ pick.name }}
            </div>
